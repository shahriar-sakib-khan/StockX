        ...
        (set, get) => ({
            ...
            hasUncommittedChanges: () => {
                ...
                return (
                    ...
                    JSON.stringify(draftSelectedBrands)
                );
            },
            ...
            toggleAllBrandsSelection: () => {
                const { draftSelectedBrands, allBrands } = get();
                const allSelected =
                    draftSelectedBrands.length === allBrands.length;
            ...
            updateDraftCylinderStock: ({ brandId, cylinderId, change }) => {
                set((state) => {
                    ...
                    const updatedDraft = state.draftSelectedBrands.map(
                        (brand) => {
                            if (brand.id !== brandId) return brand;

                            const updatedCylinders = brand.cylinders.map(
                                (cyl) => {
                                    if (cyl.id !== cylinderId) return cyl;
                                    const currentStock =
                                        typeof cyl.stock === "number"
                                            ? cyl.stock
                                            : 0;
                                    return {
                                        ...cyl,
