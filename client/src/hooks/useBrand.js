...
export const useSaveDivisionBrands = (
  workspaceId,
  divisionId,
  options = {},
) => {
  const { mutate, ...rest } = useMutation({
    mutationFn: (brandIds) =>
      saveDivisionBrands(workspaceId, divisionId, brandIds),
    onSuccess: () => {
      queryClient.invalidateQueries([DIVISION_BRANDS, workspaceId, divisionId]);
